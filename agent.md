...
## 7. 文档结构与索引（2026-08-31 规整）

**规整原则**：根目录只留 4 个活文档；产品依据进 `docs/`；过程历史进 `docs/archive/202607-process/`（47 篇）。源代码注释中引用的历史文档名（如 `CO_20260718_*.md`）一律对应归档目录。

### 7.1 现行文档（根目录）

| 文件 | 角色 |
|---|---|
| `README.md` | 用户向完整说明：启动 / 功能 / 架构 / 双远程发布 / 验证 / 打包 |
| `AGENTS.md` | 项目记忆：产品定位、一期边界、已确认决策、TRAE 督导机制 |
| `agent.md` | 本文件：开发完成态技术记录（架构决策 / 坑 / 文件索引） |
| `progress.md` | Gate 验收记录表（持续追加） |

### 7.2 现行文档（docs/）

| 文件 | 角色 |
|---|---|
| `docs/发票入账系统产品架构方案.md` | 核心产品架构（一手来源文档，670 行） |
| `docs/CO_20260718_标准凭证导入模板.csv` | 凭证 CSV 字段契约（`voucherCsvTemplate.ts` 的表头依据，勿改字段名） |
| `docs/验真与凭证规则设计.md` | 验真 / 凭证三层规则与提示词约束（2026-08-31，规则变更必须先改此文档） |

### 7.3 本次归档文档与要点（移入 docs/archive/202607-process/）

| 归档文件 | 一句话要点（内容已沉淀在本文件对应节） |
|---|---|
| `CO_20260718_本地页面验收与GateT2后端代理执行指令.md` | 录入页本地验收清单 + 后端代理实现指令 → 已落地于 §4.6 与 backend-smoke |
| `CO_20260718_腾讯云OCR验真与标准凭证导入方案.md` | OCR/验真接口选型（VatInvoiceOCR）与凭证 CSV 模板设计 → §4.6、docs/…模板.csv；推荐文档链接仍被 `defaultConfigs.ts` 引用 |
| `CO_Gate2B多发票状态机与API配置方案.md` | 多发票状态机与集成配置卡设计 → §4.1；配置存储（integrationConfigStore）仍服务侧栏通道指示 |
| `TR_TRAE_GATE_T2_BACKEND_PROXY_TASK.md` | TRAE 执行的后端代理任务书（安全约束：不打印密钥等） → 已固化在 server/ 各文件头注释 |
| `TR_TRAE_TAX_AMOUNT_FORMULA_FIX_TASK.md` | 税额公式修正任务书 → 结论在 `taxCalc.ts`（税额 = 价税合计 − 价税合计 ÷(1+税率)） |
| `findings.md` | 项目定位与产品骨架发现记录 → 全部并入 `AGENTS.md` |
| `task_plan.md` | 任务计划与模块状态表 → 全部并入本文件 §2/§3 |
| `CO_20260719_文档瘦身索引.md` | 上一轮文档瘦身索引 → 被本节取代 |

> 其余 39 篇 2026-07 过程文档（Gate 验收 / 质量复核 / 整改方案）此前已在归档目录，不再逐一摘要；需要时按文件名日期检索。
